...
var Transaction = function (_EventEmitter) {
  ...
  Transaction.prototype.commit = function commit(conn, value) {
    ...
    return this.query(conn, 'COMMIT;', 1, value).then(function () {
      _this2.trxClient.emit('committed');
  ...
  Transaction.prototype.query = function query(conn, sql, status, value) {
    ...
    var q = this.trxClient.query(conn, sql).catch(function (err) {
      status = 2;
      value = err;
      _this5._completed = true;
      debug('%s error running transaction query', _this5.txid);
    }).tap(function () {
      if (status === 1) _this5._resolver(value);
      if (status === 2) _this5._rejecter(value);
    });
